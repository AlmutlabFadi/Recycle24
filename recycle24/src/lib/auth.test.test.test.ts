...
describe('auth.ts', () => {
  ...
  it('should return demo user if credentials are correct in demo mode', async () => {
    ...
    const credentials: { email: string; password: string } = { email: "demo@example.com", password: "123456" };
    ...
    const result = await CredentialsProvider({
      ...
      async authorize(credentials: { email?: string; phone?: string; password?: string }): Promise<any> {
        if (!credentials) {
  ...
  it('should throw error if credentials are incorrect in demo mode', async () => {
    process.env.NODE_ENV = "development";
    const credentials: { email: string; password: string } = { email: "demo@example.com", password: "wrongpassword" };
    vi.mock("@/lib/demo-data", () => ({
      isDemoMode: true,
      DEMO_USERS: {
        "demo@example.com": {
  ...
  it('should return user from database if credentials are correct', async () => {
    const credentials: { email: string; password: string } = { email: "user@example.com", password: "password123" };
    const expectedUser = {
      id: "1",
      name: "User",
  ...
  it('should throw error if credentials are incorrect', async () => {
    const credentials: { email: string; password: string } = { email: "user@example.com", password: "wrongpassword" };
    vi.mock("@/lib/db", () => ({
      db: {
        user: {
