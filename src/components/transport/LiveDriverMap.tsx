...
type Props = {
    ...
    shipmentPoints?: { lat: number; lng: number; trackingId: string; status: string; driverName?: string | null }[];
...
export default function LiveDriverMap({ drivers, selectedId, routePoints, densityPoints, shipmentPoints, onSelect }: Props) {
    const driversWithCoords = drivers.filter((driver) => typeof driver.lastLat === "number" && typeof driver.lastLng === "number");

    const center = useMemo<[number, number]>(() => {
        if (!driversWithCoords.length) return mapCenterFallback;
        const lat = driversWithCoords.reduce((sum, driver) => sum + (driver.lastLat || 0), 0) / driversWithCoords.length;
        const lng = driversWithCoords.reduce((sum, driver) => sum + (driver.lastLng || 0), 0) / driversWithCoords.length;
        return [lat, lng];
    }, [driversWithCoords]);

    const selectedDriver = selectedId
        ? driversWithCoords.find((driver) => driver.driverId === selectedId)
        : null;
    ...
    return (
        <div className="w-full h-full rounded-3xl overflow-hidden border border-slate-800">
            <MapContainer center={center} zoom={8} scrollWheelZoom className="w-full h-full">
                <TileLayer
                ...
                {selectedDriver && (
                    <FlyToDriver position={[selectedDriver.lastLat as number, selectedDriver.lastLng as number]} />
                )}
                ...
                {densityPoints?.map((point, idx) => (
                    <CircleMarker
                        ...
                        pathOptions={{ color: "#38bdf8", fillColor: "#38bdf8", fillOpacity: 0.15, opacity: 0.2 }}
                    />
                ))}
                        ...
                        >
                            <Popup>
                                <div className="text-xs">
                                    ...
                                    {driver.lastSeenAt && (
                                        <p>{new Date(driver.lastSeenAt).toLocaleTimeString("ar-SY", { hour: "2-digit", minute: "2-digit" })}</p>
                                    )}
                                </div>
                            </Popup>
                        </CircleMarker>
