...
export default function SafetyLibraryDetailPage() {
    ...
    useEffect(() => {
        const fetchItem = async () => {
            try {
                ...
                console.error("Safety library detail error:", error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleCopy = async () => {
        try {
            ...
            setCopyMessage("تم نسخ رابط المنشور");
    ...
    return (
        <div className="flex flex-col min-h-screen bg-bg-light dark:bg-bg-dark font-display pb-24">
            <HeaderWithBack title="تفاصيل المحتوى" />
                    ...
                    <article className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                        ...
                        <div className="p-5 space-y-3">
                            ...
                            {item.mediaUrl && item.type === "VIDEO" && (
                                <video src={item.mediaUrl} controls className="w-full rounded-xl" />
                            )}
                            {item.mediaUrl && item.type === "IMAGE" && (
                                <a href={item.mediaUrl} target="_blank" rel="noreferrer">
                                    <img src={item.mediaUrl} alt={item.title} className="w-full rounded-xl" />
                                </a>
                            )}
                            {item.mediaUrl && item.type !== "VIDEO" && item.type !== "IMAGE" && (
                            ...
                            <div className="mt-4 border-t border-slate-200 dark:border-slate-800 pt-4">
                                <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-2">مشاركة المنشور</h4>
                                {copyMessage && (
