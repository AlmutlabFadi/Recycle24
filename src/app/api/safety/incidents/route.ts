...
export async function POST(request: NextRequest) {
  try {
        ...
        data: {
          ...
          longitude:
            typeof parsedLongitude === "number" && Number.isFinite(parsedLongitude)
              ...
              : null,
          locationAccuracy:
            typeof parsedAccuracy === "number" && Number.isFinite(parsedAccuracy)
    ...
    const mapLink = hasCoords
      ? `https://maps.google.com/?q=${parsedLatitude},${parsedLongitude}`
      : "";
    ...
    const messageLines = [
      ...
      typeof parsedAccuracy === "number" && Number.isFinite(parsedAccuracy)
        ? `دقة الموقع: ${Math.round(parsedAccuracy)}م`
        : "",
      `الوصف: ${description}`,
