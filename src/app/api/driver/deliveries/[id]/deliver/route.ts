  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const pod = await tx.proofOfDelivery.create({
        data: {
          deliveryId,
          signedName: typeof signedName === "string" ? signedName : null,
          signatureSvg: typeof signatureSvg === "string" ? signatureSvg : null,
          photoUrls,
    ...
    const status =
      message === "DELIVERY_NOT_FOUND"
        ? 404
        : message === "INVALID_STATE" || message === "POD_PHOTOS_REQUIRED"
        ? 400
        : 500;
