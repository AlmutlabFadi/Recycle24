...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ success: false, error: "تعذر تحميل بيانات الاشتراكات" }, { status: 500 });
    }
}
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { userId, plan, status, endDate, price } = await request.json();
        ...
        return NextResponse.json({ success: false, error: "تعذر تحديث الاشتراك" }, { status: 500 });
    }
}
