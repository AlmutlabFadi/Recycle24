...
function PaymentContent() {
  ...
  const calculateTotal = () => {
    if (!pkg) return 0;
    return billingCycle === "yearly" ? Math.floor(pkg.price * 12 * 0.8) : pkg.price;
  };
  ...
  if (!pkg) {
    return (
      <div className="min-h-screen bg-bg-dark flex flex-col items-center justify-center p-6 text-center">
        <span className="material-symbols-outlined !text-6xl text-red-500 mb-4">error</span>
        <h2 className="text-xl font-bold text-white mb-2">الباقة غير متاحة</h2>
        <button
          onClick={() => router.push("/subscription")}
          className="bg-primary text-white px-6 py-2 rounded-xl"
        >
  ...
  const basePrice = pkg.price * (billingCycle === "yearly" ? 12 : 1);
  const yearlyDiscount = billingCycle === "yearly" ? pkg.price * 12 * 0.2 : 0;
        ...
        <div className="bg-surface-highlight rounded-[2.5rem] border border-slate-800 p-8 shadow-2xl mb-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-slate-400">
                نظام الفوترة: {billingCycle === "monthly" ? "شهري" : "سنوي"}
              </p>
            </div>
          ...
          <div className="space-y-4 pt-6 border-t border-slate-800/50">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white font-english">
            ...
            {billingCycle === "yearly" && (
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-500 font-english">
                  - {yearlyDiscount.toLocaleString()} ل.س
                </span>
              </div>
            )}
            ...
            <div className="flex justify-between text-lg font-bold pt-4 border-t border-slate-800">
              ...
              <span className="text-primary font-english">
        ...
        <div className="bg-surface-highlight rounded-[2.5rem] border border-slate-800 p-8 shadow-2xl">
          ...
          <div className="space-y-3">
            {["الرصيد في المحفظة", "بطاقة ائتمان", "دفع عند نقطة توزيع"].map((method, idx) => (
              <label
                key={idx}
                className="flex items-center gap-4 p-4 rounded-2xl bg-slate-900 border border-slate-800 cursor-pointer hover:border-primary transition"
              >
...
export default function PaymentPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-bg-dark flex items-center justify-center animate-pulse text-primary italic">
          Recycle24 Payment Gateway Loading...
        </div>
      }
    >
      <PaymentContent />
